# Payments: RedFace Skool uses RedFace Pay

RedFace Skool is a **commerce client**. RedFace Pay is the **payment infrastructure**.

Students and instructors never see Paystack in RedFace Skool. Card UI, if any, lives on hosted RedFace checkout.

## Contract

```text
RedFace Skool offer (course / live / event ticket)
      ↓
RedFace Skool enrollment (pending)
      ↓
...
GET /payments/:id until status = paid
      ↓
RedFace Skool enrollment = paid (access)
```
...
Settlement split to instructors is a **RedFace Pay / subaccount** concern later. v1 records the intended share in metadata. Do not build a second ledger in this app.
...
| `REDFACE_BUSINESS_ID` | RedFace Skool platform merchant on RedFace |
...
Default marketplace share: instructor 80%, RedFace Skool 20%. Memberships, instructor Pro, events take, and corporate seats are later SKUs. Do not implement subscriptions in v1.
